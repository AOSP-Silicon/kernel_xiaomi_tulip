...
DEFCONFIG="vendor/xiaomi/sdm660_defconfig vendor/xiaomi/tulip.config kernelsu-next.config"
OUT_DIR="$(pwd)/out"
...
function build_kernel() {
    DATE=$date
    BUILD_START=$(date +"%s")
    make ARCH=arm64 CC=$COMPILER_NAME LD=$LD_NAME CROSS_COMPILE=$CROSS_COMPILE_ARM64 CROSS_COMPILE_COMPAT=$CROSS_COMPILE_COMPAT $CMDS O=$OUT_DIR $DEFCONFIG
    make ARCH=arm64 CC=$COMPILER_NAME LD=$LD_NAME CROSS_COMPILE=$CROSS_COMPILE_ARM64 CROSS_COMPILE_COMPAT=$CROSS_COMPILE_COMPAT $CMDS O=$OUT_DIR -j$(nproc --all) |& tee -a $HOME/build/build$BUILD.txt
    BUILD_END=$(date +"%s")
